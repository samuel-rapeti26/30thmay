...
# function to verify user for forget password
def user_auth(data):
    # set connection
    con_name = conf_file['db_access']["container_user"]
    container = database.get_container_client(con_name)
    ...
    if len(user) != 0:
        # select first user
        user = user[0]
        # user's PIN is stored in db and match PIN in db to the one that user send
        salt1 = bytes(user["PasswordSalt"], 'latin1')
        hashedpin = hashpass(data["PIN"], salt1)
        if hashedpin == bytes(user["PIN"], 'latin1'):   
...
def store_pass(data):
    ...
    if len(user) != 0:
        ...
        if len(data["New_Password"]) >= 8:
            ...
            salt1 = bytes(user["PasswordSalt"], 'latin1')
            new_hashpassword = hashpass(data["New_Password"], salt1).decode('latin1')
            #print("*"*100)
            #print(new_hashpassword)
            #print("*"*100)
            #print(hashpass(data["New_Password"], salt1))
            
            user = container.replace_item(item = user, body = {"id": user["id"], "User": user["User"], "Password": new_hashpassword,
            "Admin": user["Admin"], "PasswordSalt": user["PasswordSalt"], "PIN": user["PIN"]})
            #operations =[{"op": 'replace', "path": '/Password', "value": new_hashpassword}]
            #response = container.patch_item(item=user, partition_key = azure.cosmos.PartitionKey(path = "/Admin"), patch_operations=operations)
            return True, "Password updated."
        else:
            return False, "Enter Password with minimum 8 characters."
    else:
        return False, "User ID not found."
...
def prepareData(inputData):
    ...
    #     print("LATER", inputData1)
        ...
        dosage = list(dosageFreq[None]["Code"])
        ...
        for p in range(len(index_of_occ)):
            if len(index_of_occ[p]) > 0:                   
                for i in range(len(index_of_occ[p])):
                    ...
                    if ABB2 in accepted_abbrev or ABB2 in dosage:     
                        continue
                    h_GB=Hunspell('en_GB')
                    h_en=Hunspell('en_US')
                    if h_GB.spell(ABB2) or h_en.spell(ABB2):
                        continue
                    ...
                    if re.findall(expansion_pat, inputData[p]) != []:
                        valid_extr_char_cnt = len(re.findall("\\w", re.findall(expansion_pat, inputData[p])[0][0])) #CHANGE
                    else:
        ...
        for i in dsg_fr2:
            ...
            for j in range(len(inputData)):
                ...
                for k in replacedWord:
                    ...
                    suggest = [''] if pd.isna(suggest[0]) else suggest
                    suggestedWord = suggestedWord + suggest
                    startPos=startPos + [k.start()]
                    endPos=endPos + [k.end()]
        ...
        def extract_plan(text):
            for i in range(len(text)):
                for j in range(i+1, len(text)):
                    if re.sub(r'[^\w\s]', '',text[i]).strip().lower() in ["plan", "plans"] and re.sub(r'[^\w\s]', '',text[j]).strip().lower() in ["attestation", "followup followup", "followup", "orders", "order", "instructions"]:
                        return text[i+1:j]
        ...
        if len(wordTable) == 0:
            ...
            wordTable = wordTable.sort_values(by=['ParagraphNum','StartPos']).reset_index(drop=True)
            
            drop_ind = []
            for p in wordTable['ParagraphNum'].unique():
                if len(wordTable[wordTable['ParagraphNum'] == p]) > 1:
                    for r in wordTable[wordTable['ParagraphNum'] == p].index[1:]:
                        if (wordTable.iloc[r, 5] <= wordTable.iloc[r-1, 6]) and (wordTable.iloc[r, 6] <= wordTable.iloc[r-1, 6]):
                            drop_ind.append(r)
                            
            wordTable = wordTable.drop(drop_ind)             
            return (wordTable.reset_index(drop=True)).to_dict()
