...
# user authentication for forget password
...
def auth_user():
    ...
    if request.method == "POST":
        ...
        # call function to validate the user  
        result, msg = user_auth(request_body)
        return jsonify({"Is_verified":result, "message": msg})
...
def new_pass():
    ...
    if request.method == "POST":
        ...
        # call function to change user password in DB  
        result, msg = store_pass(request_body)
        return jsonify({"password_change":result, "message": msg})
